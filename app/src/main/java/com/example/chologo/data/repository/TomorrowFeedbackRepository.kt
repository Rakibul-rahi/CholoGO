...
/**
 * Mirrors RideNowFeedbackRepository exactly, but writes back onto
 * ride_requests (Tomorrow flow) instead of ride_now_requests.
 */
class TomorrowFeedbackRepository(
    ...
    suspend fun submitRideRating(rating: RideRating): Result<String> {
        return try {
            ...
            val requestDoc = rideRequestsRef.document(rating.requestId)
    ...
    /** The Tomorrow-flow mirror of [submitRideRating] - see RideNowFeedbackRepository.submitPassengerRating. */
    ...
    suspend fun submitRideReport(report: RideReport): Result<String> {
        return try {
            ...
            val requestDoc = rideRequestsRef.document(report.requestId)
